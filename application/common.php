...
function getToken($user_id){
    $token = \think\Db::name('user_token')->where('user_id', $user_id)->find();
    $new_token = createToken($user_id);
    if($token){
        \think\Db::name('user_token')->where('user_id', $user_id)->update(['token'=>$new_token]);
    }else{
        \think\Db::name('user_token')->insert(['user_id'=>$user_id, 'token'=>$new_token]);
    }
    return $new_token;
}

function getUserIdByToken($user_id){
    // return $user_id;  // 原样返回
    $token = \think\Db::name('user_token')->where('token',$user_id)->find();
    if($token){
