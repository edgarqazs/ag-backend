...
var app = new Vue({
	...
	created() {
		var that=this;
		that.getTxData()
		$.post(this.baseUrl+'/index/Kline/aglog_group',function(json){
			that.group_data=json.data.reverse();
			that.getDepth()
			that.getKline()
		});
	...
	methods:{
		...
		getTxData(){
			var that=this;
			$.post(this.baseUrl+'/index/Kline/getTxData',function(json){
				that.txData=json.data; 
			});
		},
		
		// 获取k线数据,生成k线
		getKline(){
			//var dataMA5 = this.calculateMA(5, this.group_data,'agv_price');
			
			var dataMA10 = this.calculateMA(10, this.group_data,'agv_price');
			var dataMA30 = this.calculateMA(30, this.group_data,'agv_price');
			var volumeMA5 = this.calculateMA(5, this.group_data,'sum_trans_amount');
			var volumeMA10 = this.calculateMA(10, this.group_data,'sum_trans_amount');
			var data= this.group_data.map(function(item) {
				return [item['begin_price'],item['max_price'],item['min_price'],item['end_price'],item['sum_trans_amount']];
			});
			var dataMA5 =this.group_data.map(function(item) {
				return item['agv_price'];
			});
			var dates = this.group_data.map(function(item) {
				var temp=item['date_day'].split('-');
				temp.shift();
				return temp.join('-');
			});
			var volumes=this.group_data.map(function(item, index) {
				return [index, item['sum_trans_amount'],1];
			});
			myChart.setOption({
				xAxis: [  
					{ 
		...
		getDepth(){
			...
			for(var i=data.length-1;i>=0;i--){
				var item=data[i];
				var sum_trans_count=item.sum_trans_count;
				var date=item.date_day.split('-');
				date.shift();
				date=date.join('-');
				that.buyList.push({
					"date":date,
					"price": item.agv_price,
					"amount": item.sum_buy_count,
					'width':parseInt(item.sum_buy_count*100/sum_trans_count)
				})
				that.sellList.push({
					"date":date,
					"price": item.agv_price,
					"amount": item.sum_sell_count,
					'width':parseInt(item.sum_sell_count*100/sum_trans_count)
				})
			}
		...
		getDealHis(){
			this.dealHis=[];
			var that=this;
			$.post(this.baseUrl+'/index/Kline/aglog',function(json){
				var data=json.data;
				for(var i in data){
					that.dealHis.push({
						"date": data[i].ctime,
						"takerFlag": data[i].type==1?'1':'2',
						"price": data[i].price,
						"amount": data[i].num
					});
				}
		...
		// 计算MA
		calculateMA(dayCount, data,field) {
			var result = [];
			for (var i = 0, len = data.length; i < len; i++) {
				...
				for (var j = 0; j < dayCount; j++) {
					//console.log(data[i - j][field])
					sum += data[i - j][field]*1;
				}
